# Modified from https://github.com/open-mmlab/mmdetection/blob/v2.25.3/mmdet/core/optimizers/layer_decay_optimizer_constructor.py
...
from mmdet.core.optimizers import OPTIMIZER_BUILDERS
...
class RDNetBackbone(BaseModule):
    def __init__(self,
        ...
        self.norm_modules = nn.ModuleList(
            [nn.GroupNorm(num_groups=1, num_channels=channels[i]) for i in range(len(channels))]
        )
    ...
    def forward(self, x):
        ...
        features = [norm(feat) for norm, feat in zip(self.norm_modules, features)]
